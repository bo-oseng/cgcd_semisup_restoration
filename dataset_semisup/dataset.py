...
class TrainLabeled(data.Dataset):
    def __init__(self, dataroot, phase, finesize=256, degradation_subset=None):
        super().__init__()
        self.phase = phase
        self.root = dataroot
        ...
        # degradation classes
        self.degradation_classes = list(degradation_subset)
        ...
        for deg_class in tqdm.tqdm(self.degradation_classes):
            dir_input = os.path.join(self.root, "Train", deg_class)
            dir_gt = os.path.join(self.root, "GT", "clear")
            if not os.path.exists(dir_gt):
                label_root = os.path.join(self.root, "Pseudo", deg_class)
            else:
                label_root = dir_gt

            label_images = sorted(make_dataset(label_root))
            self.gt_paths.extend(label_images)
...
class TrainUnlabeled(data.Dataset):
    def __init__(
        ...
        for deg_class in tqdm.tqdm(self.degradation_classes):
            dir_input = os.path.join(self.root, "Pseudo", deg_class)
            dir_candidate = os.path.join(self.root, "Pseudo", deg_class)
            ...
            if os.path.isdir(dir_input):
                ...
                if os.path.isdir(dir_candidate):
                    ...
                    candidate_images = list(input_images)
...
class ValLabeled(data.Dataset):
    def __init__(self, dataroot, phase, finesize, mode=None, degradation_subset=None):
        super().__init__()
        self.phase = phase
        self.root = dataroot
        self.fineSize = finesize
        ...
        for deg_class in self.degradation_classes:
            ...
            dir_input = os.path.join(self.root, "Test", deg_class)
            dir_gt = os.path.join(self.root, "Test", "clear")
