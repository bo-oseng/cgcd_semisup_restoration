...
class create_ccd_dataset_w_clear(Dataset):
    """
    ...
    def __init__(self, dataset, transform, stage) -> None:
        super(create_ccd_dataset_w_clear, self).__init__()
...
class create_ccd_test_dataset_w_clear(Dataset):
    """
    Input: dataset class and splitted data index list
    ...
    def __init__(self, unlabelled_dataset, labelled_dataset, transform) -> None:
        super(create_ccd_test_dataset_w_clear, self).__init__()
...
def create_dataloader_wo_contra(args, dataset_i, stage_i):
    ...
    if stage_i == -1:
        ...
        dataset_i = create_ccd_dataset_w_clear(dataset_i, transform, stage=0)
...
def build_CCD_dataset(args, split):
    ...
    if split == "train":
        transform = data_util.get_paired_transform
