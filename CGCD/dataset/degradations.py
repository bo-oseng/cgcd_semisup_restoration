...
class DegradationDataset(BaseDataset):
    def __init__(self, root, mode, transform=None):
        ...
        old_cnt = len(os.listdir(self.path_train_o))
        new_cnt = len(os.listdir(self.path_train_n_1))
        if self.mode == "train_0":
            self.classes = range(0, old_cnt)
        ...
        self.idx2cls = dataset.classes
        for index, (img_path, label) in enumerate(dataset.imgs):
            self.labels += [label]
            self.I += [index]
