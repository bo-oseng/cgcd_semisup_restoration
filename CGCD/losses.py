...
class FreqProxy_Anchor(torch.nn.Module):
    def __init__(self, nb_classes, sz_embed, mrg=0.1, alpha=32):
        torch.nn.Module.__init__(self)
        self.proxies = torch.nn.Parameter(torch.randn(nb_classes, sz_embed).cuda())
        nn.init.kaiming_normal_(self.proxies, mode="fan_out")
    ...
    def forward(self, feat, target):
        proxy = torch.abs(torch.fft.fft(self.proxies, dim=-1))
        feat = torch.abs(torch.fft.fft(feat.cuda(), dim=-1))
        cos = F.linear(l2_norm(feat), l2_norm(proxy))  # Calcluate cosine similarity

        P_one_hot = binarize(T=target, nb_classes=self.nb_classes)
        N_one_hot = 1 - P_one_hot
...
class Proxy_NCA(torch.nn.Module):
    def __init__(self, nb_classes, sz_embed, scale=32):
        ...
        self.loss_func = losses.ProxyNCALoss(
            num_classes=self.nb_classes, embedding_size=self.sz_embed, softmax_scale=self.scale
        ).cuda()
