...
from OneRestore.utils.utils_word_embedding import initialize_wordembedding_matrix
...
class Backbone(
    ...
    repo_url="https://github.com/gy65896/OneRestore",
...
class Embedder(nn.Module):
    ...
    def _setup_word_embedding(self):
        ...
        # my code, for 000.clear
        self.clean_up_type_name = [remove_num.split(".")[-1] for remove_num in self.type_name]
        self.cleantype2idx = {self.clean_up_type_name[i]: i for i in range(len(self.clean_up_type_name))}
        wordemb, self.word_dim = initialize_wordembedding_matrix(self.wordembs, self.clean_up_type_name)

        self.embedder = nn.Embedding(self.num_type, self.word_dim)
        self.embedder.weight.data.copy_(wordemb)

        self.mlp = nn.Sequential(nn.Linear(self.word_dim, self.out_dim), nn.ReLU(True))
    ...
    def image_encoder_forward(self, batch):
        ...
        text_type = [self.clean_up_type_name[num_type[i]] for i in range(bs)]
    ...
    def text_encoder_forward(self, text):
        ...
        for i in range(bs):
            num_type[i] = self.cleantype2idx[text[i]]
