...
class PGFM(nn.Module):
    def __init__(self, channel=32, q_dim=324):
        super(PGFM, self).__init__()
        ...
        self.encoder = encoder(channel, q_dim=q_dim)
        self.middle = backbone(channel, q_dim=q_dim)
        self.decoder = decoder(channel, q_dim=q_dim)
...
class encoder(nn.Module):
    def __init__(self, channel, q_dim=324):
        super(encoder, self).__init__()

        self.el = ResidualBlock(channel, q_dim=q_dim)  # 16
        self.em = ResidualBlock(channel * 2, q_dim=q_dim)  # 32
        self.es = ResidualBlock(channel * 4, q_dim=q_dim)  # 64
        self.ess = ResidualBlock(channel * 8, q_dim=q_dim)  # 128
        ...
        self.conv_estess = nn.Conv2d(
            4 * channel, 8 * channel, kernel_size=1, stride=1, padding=0, bias=False
        )  # 64 128
        self.conv_esstesss = nn.Conv2d(
            8 * channel, 16 * channel, kernel_size=1, stride=1, padding=0, bias=False
        )  # 128 256
...
class backbone(nn.Module):
    def __init__(self, channel, q_dim=324):
        ...
        self.s1 = ResidualBlock(channel * 8, q_dim=q_dim)  # 128
        self.s2 = ResidualBlock(channel * 8, q_dim=q_dim)  # 128
...
class decoder(nn.Module):
    def __init__(self, channel, q_dim=324):
        ...
        self.dss = ResidualBlock(channel * 8, q_dim=q_dim)  # 128
        self.ds = ResidualBlock(channel * 4, q_dim=q_dim)  # 64
        self.dm = ResidualBlock(channel * 2, q_dim=q_dim)  # 32
        self.dl = ResidualBlock(channel, q_dim=q_dim)  # 16

        self.conv_dsstds = nn.Conv2d(
            8 * channel, 4 * channel, kernel_size=1, stride=1, padding=0, bias=False
        )  # 128 64
...
class ResidualBlock(nn.Module):
    def __init__(self, channel, norm=False, q_dim=324):
        super(ResidualBlock, self).__init__()

        self.el = TransformerBlock(
            channel, num_heads=8, ffn_expansion_factor=2.66, bias=False, LayerNorm_type="WithBias", q_dim=q_dim
        )
...
class Cross_Attention(nn.Module):
    def __init__(self, dim, num_heads, bias, q_dim=324):
...
# PGFM
class PromptGuidedFeatureMoodule(nn.Module):
    def __init__(self, dim, q_dim):
        super(PromptGuidedFeatureMoodule, self).__init__()
        self.q_dim = q_dim
        self.mlp = nn.Sequential(nn.Linear(q_dim, dim * 2), nn.ReLU(), nn.Linear(dim * 2, dim * 2))
        self.gamma = nn.Parameter(torch.zeros(1, dim, 1, 1), requires_grad=False)
        self.beta = nn.Parameter(torch.zeros(1, dim, 1, 1), requires_grad=False)
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads=8, ffn_expansion_factor=2.66, bias=False, LayerNorm_type="WithBias", q_dim=324):
        super(TransformerBlock, self).__init__()
        self.norm1 = LayerNorm(dim, LayerNorm_type)
        self.pgfm1 = PromptGuidedFeatureMoodule(dim, q_dim=q_dim)
        self.cross_attn = Cross_Attention(dim, num_heads, bias, q_dim=q_dim)

        self.norm2 = LayerNorm(dim, LayerNorm_type)
        self.pgfm2 = PromptGuidedFeatureMoodule(dim, q_dim=q_dim)
        self.self_attn = Self_Attention(dim, num_heads, bias)

        self.ffn = FeedForward(dim, ffn_expansion_factor, bias)
        self.pgfm3 = PromptGuidedFeatureMoodule(dim, q_dim=q_dim)
        self.norm3 = LayerNorm(dim, LayerNorm_type)

    def forward(self, x, query):
        x = x + self.cross_attn(self.pgfm1(self.norm1(x), query), query)
        x = x + self.self_attn(self.pgfm2(self.norm2(x), query))
        x = x + self.ffn(self.pgfm3(self.norm3(x), query))
        return x


if __name__ == "__main__":
    net = PGFM().to("cuda" if torch.cuda.is_available() else "cpu")
